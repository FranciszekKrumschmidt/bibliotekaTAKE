...
public class KatalogService {
    ...
    public Kategoria getKategoria(Long id) {
        return kategoriaRepository.findById(id).orElseThrow(() -> new RuntimeException("Nie znaleziono kategorii"));
    }

    public Tytul getTytul(Long id) {
        return tytulRepository.findById(id).orElseThrow(() -> new RuntimeException("Nie znaleziono tytułu"));
    }

    @Transactional
    public Kategoria edytujKategorie(Long id, String nazwa) {
        Kategoria kategoria = getKategoria(id);
        kategoria.setNazwa(nazwa);
        return kategoriaRepository.save(kategoria);
    }

    @Transactional
    public Tytul edytujTytul(Long id, String tytulNazwa, String autor, LocalDateTime dataWydania, Long kategoriaId) {
        Tytul tytul = getTytul(id);
        Kategoria kategoria = getKategoria(kategoriaId);
        tytul.setTytul(tytulNazwa);
        tytul.setAutor(autor);
        tytul.setDataWydania(dataWydania);
        tytul.setKategoria(kategoria);
        return tytulRepository.save(tytul);
    ...
    @Transactional
    public void usunTytul(Long id) {
        tytulRepository.deleteById(id);
    }

    @Transactional
    public void usunWolumen(Long id) {
        wolumenRepository.deleteById(id);
    }
}
